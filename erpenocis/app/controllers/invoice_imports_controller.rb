    ...
    begin
      ...
      if @invoice_import.save
        if params[:invoice_import][:proj_id]
          redirect_to project_path(params[:invoice_import][:proj_id],show: params[:invoice_import][:show],st: params[:invoice_import][:st]), notice: "Imported invoices successfully."
        else
